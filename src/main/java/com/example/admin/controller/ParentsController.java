...
public class ParentsController {
    ...
    public ParentsResponse getMember (@Parameter(description = "사용자 ID", example = "user_Id") @PathVariable("userId") String userId) {
        Parents parents = parentsService.findByUserId(userId);
        return new ParentsResponse(parents.getUserId(), parents.getNickname(), parents.getPhoneNumber(), parents.getEmail());
    }

    @PutMapping("/parents/") //적당한 url 작성필요(사용자의 정보를 수정하는 기능)
    public ParentsResponse changeMemberInfo(@RequestBody ParentsUpdateRequest request) {
        Parents findParents  = parentsService.changeInfo(request.getToken(), request.getNickname(), request.getPhoneNumber(), request.getEmail());
        return new ParentsResponse(findParents.getUserId(), findParents.getNickname(), findParents.getPhoneNumber(), findParents.getEmail());
    }
